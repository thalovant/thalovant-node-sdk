...
- New exports: `HubIntentInventory`, `HubSkillIntents`, `HubIntent`, `IntentRegistration`, `IntentDefinition`, `DescribeTarget`, `HubIntentSource`, `SOURCE_MANIFEST`, `SOURCE_ENGINES`, the low-level `requestReply`, `listIntents`, `describeIntent`, `describeMany`, `describeKey`, `intentNames` and `intentInventory` helpers, and the event names `EVENT_INTENT_LIST`, `EVENT_INTENT_LIST_RESPONSE`, `EVENT_INTENT_DESCRIBE`, `EVENT_INTENT_DESCRIBE_RESPONSE`, `EVENT_ADAPT_MANIFEST_GET`, `EVENT_ADAPT_MANIFEST`, `EVENT_PADATIOUS_MANIFEST_GET` and `EVENT_PADATIOUS_MANIFEST`. No existing signature changed and the browser bundle is unaffected.
